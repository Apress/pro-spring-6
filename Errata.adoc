...
On *page 72* the output in Listing 3-27 should be:

[source, log]
----
song1: from parent ctx: Gravity
song2: from parent ctx: Daughters
song3: from child ctx: No Such Thing
----

(Submitted by https://github.com/XPD-Kasun[XPD-Kasun])
...
On *page 81* there is a mistake in the fourth paragraph of the `Customizing Bean Naming Style`.

`@Configuration` should be `@ComponentScan`.

(Submitted by _Raj Sarkar_)
...
On **page 292**, the first paragraph after Listing 6-23, mentions the `assertTrue(..)` being used in the said listing. It should be `assertEquals()`.
...
== Chapter 17: Securing Spring Applications
